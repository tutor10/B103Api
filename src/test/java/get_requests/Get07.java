...
public class Get07 extends JsonPlaceHolderBaseUrl {
    ...
    public void get07() {
       ...
       // 2)Print all ids greater than 190 on the console ==> id si 190 dan buyuk olanlarin id lerini konsola yazdirin
        JsonPath jsonPath = response.jsonPath();
        List<Integer> list = jsonPath.getList("findAll{it.id>190}.id");//Groovy language("findAll{it.id==200}.title") Java temelli bir programlama dilidir.
        ...
        // 3)Print all userIds whose ids are less than 5 on the console ==> id si 5 den kucuk olan tum userid lerini konsolunu yazdirin
        List<Integer> userIdList = jsonPath.getList("findAll{it.id<5}.userId");//Groovy language("findAll{it.id==200}.title") Java temelli bir programlama dilidir.
        ...
        //4)Print all titles whose ids are less than 5 ==> ıd si 5 den kucuk olan tum basliklari yazdirin
        List<String> titleList = jsonPath.getList("findAll{it.id<5}.title");//Groovy language("findAll{it.id==200}.title") Java temelli bir programlama dilidir.
